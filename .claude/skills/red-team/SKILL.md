...
name: red-team
description: >-
  Multi-vector adversarial code review that strips sympathetic context and
  attacks from security, engineering, architecture, dependency, and testing
  angles simultaneously. Mandatory findings -- rubber-stamp approvals are
  not possible. Composes with adversarial-review, security-audit,
  dependency-audit, anti-patterns, testing-standards, and code-review skills.
  TRIGGER when: user says "red team", asks for adversarial review, wants
  aggressive code critique, requests finding everything wrong with a project,
  or explicitly prioritizes finding problems over politeness.
  DO NOT TRIGGER when: user wants a standard code review (use code-review),
  is debugging a specific bug (use systematic-debugging), wants to understand
  code flow (use xray), or is asking for constructive feedback on approach.
---
