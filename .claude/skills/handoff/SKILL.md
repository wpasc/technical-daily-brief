...
name: handoff
description: >-
  Preserve session continuity in Linear with sub-agent drafting and one
  handoff scale: either a lightweight checkpoint comment or a fuller handoff
  document for another agent/session to resume from. The skill chooses the
  mode, keeps summarization context out of the main agent, and writes the
  durable result to the active Linear Issue.
  TRIGGER when: the user signals a pause or context switch ("stepping away",
  "wrap this up", "switching to X", "let's come back to this"); asks for a
  "handoff", "checkpoint", or "save state"; a multi-step task reaches a
  natural stopping point worth preserving; substantive work has produced
  uncommitted changes the user should review before moving on.
