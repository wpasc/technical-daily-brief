...
description: >-
  Phased feature implementation -- scout the codebase, clarify requirements,
  plan with per-step verification, implement test-first, review before
  handing back. Principles and exit criteria only; mechanics delegate to
  native subagents and sibling skills.
  TRIGGER when: user requests a feature that spans multiple files or
  subsystems, requirements need clarification before code, or the change
  warrants a structured build-and-review loop.
  DO NOT TRIGGER when: quick bug fixes, documentation-only changes, simple
  refactors with known solutions, or single-file changes with obvious scope.
---
