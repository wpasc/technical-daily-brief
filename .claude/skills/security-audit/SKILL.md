...
name: security-audit
description: >-
  OWASP Top 10 audit methodology, attack surface enumeration, and vulnerability
  pattern detection for web applications and APIs. Covers injection, auth bypass,
  data exposure, secret detection, and API-specific attack patterns.
  TRIGGER when: reviewing code that handles authentication, authorization, user
  input, API endpoints, session management, cryptographic operations, or when
  security-focused review is requested.
  DO NOT TRIGGER when: reviewing purely internal logic with no external trust
  boundaries, UI-only code with no API interaction, or running a full red team
  (use red-team which composes with this skill).
